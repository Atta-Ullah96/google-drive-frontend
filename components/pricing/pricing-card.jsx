...
export default function PricingCard({ plan, buttonLabel, onSelect, disabled = false, loading = false }) {
  ...
  return <article className={`relative flex h-full flex-col rounded-lg border bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-lg ${plan.popular ? "border-blue-500 ring-2 ring-blue-100" : "border-gray-200"}`}>
    ...
    <button type="button" onClick={() => onSelect(plan)} disabled={disabled || loading} className={`mt-7 flex h-11 items-center justify-center rounded-md text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 ${buttonStyles}`}>{loading ? "Loading..." : buttonLabel || plan.cta}</button>
  </article>;
}
