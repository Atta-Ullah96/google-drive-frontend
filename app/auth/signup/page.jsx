...
import { googleAuth, register } from "@/lib/api/auth";
...
const PERKS = [
  { emoji: "🔒", label: "Private & secure by default" },
  { emoji: "📂", label: "Organize with folders & tags" },
  { emoji: "🌐", label: "Share links with anyone" },
  { emoji: "📱", label: "Works on mobile & desktop" },
];
      ...
      <div className="hidden lg:flex lg:w-[460px] xl:w-[520px] shrink-0 flex-col p-12 relative overflow-hidden bg-[#1a73e8]">
        ...
        <div className="relative z-10 mt-auto">
          ...
          <p className="text-blue-100 text-[15px] leading-relaxed mb-10 max-w-xs">
            Join thousands of people who trust Storix to keep their files safe and within reach.
          </p>

          <ul className="grid grid-cols-2 gap-3">
            {PERKS.map(({ emoji, label }) => (
              <li key={label} className="flex items-center gap-2.5 bg-white/10 rounded-xl px-3.5 py-3">
                <span className="text-lg leading-none">{emoji}</span>
                <span className="text-blue-50 text-xs font-medium leading-snug">{label}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10 sm:px-10 bg-white">
        <div className="w-full max-w-[400px]">
          ...
          <div className="mb-7">
            ...
            <p className="text-gray-500 text-sm mt-1">Start with 15 GB of free storage</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1.5">Use 8 or more characters with letters and numbers</p>
            </div>
          ...
          <p className="text-center text-sm text-gray-500 mt-4">
            ...
            <Link href="/auth/login" className="text-[#1a73e8] hover:underline font-medium">
              Sign in
            </Link>
          </p>
        </div>
